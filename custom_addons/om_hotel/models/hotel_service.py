...
class HotelService(models.Model):
    ...
    is_free = fields.Boolean(string="Is Free Service", compute="_compute_is_free", store=True)

    @api.depends('service_line_ids.unit_price')
    def _compute_is_free(self):
        """Kiểm tra nếu tất cả dịch vụ đều miễn phí"""
        for service in self:
            service.is_free = all(line.unit_price == 0 for line in service.service_line_ids)
    ...
    def _update_inventory(self):
        """Cập nhật tồn kho khi xác nhận dịch vụ"""
        for line in self.service_line_ids:
    ...
    def create_sale_order(self):
        for service in self:
            # """Tạo đơn hàng bán (Sale Order) dựa trên các dòng dịch vụ"""
            self.ensure_one()
            order_lines = []
