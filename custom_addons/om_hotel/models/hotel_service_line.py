...
class HotelServiceLine(models.Model):
    ...
    booking_id = fields.Many2one('hotel.management.booking', string='Booking', ondelete ='cascade')
    ...
    def deduct_inventory(self):
        """Trừ tồn kho khi sử dụng sản phẩm"""
        for line in self:
            if line.product_id.type in ['consu', 'product'] and line.quantity > 0:
                ...
                if available_qty < line.quantity:
                    raise UserError(("Not enough stock for product %s. Available: %s, Required: %s") % (
